    ...
    namespace QLBH_Dion.Controllers
    {
        [Route("[controller]")]
        [ApiController]
        public class AuctionProductStatusController : BaseController
        {
            IAuctionProductStatusService service;
            public AuctionProductStatusController(IAuctionProductStatusService _service, ICacheHelper cacheHelper )
            {
                service = _service;
            }
            [HttpGet]
            ...
            [HttpPost]
            [Route("api/Add")]
            public async Task<IActionResult> Add([FromBody]AuctionProductStatus model)
            {
            ...
            [HttpPost]
            [Route("api/Update")]
            public async Task<IActionResult> Update([FromBody]AuctionProductStatus model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            [HttpPost]
            [Route("api/Delete")]
            public async Task<IActionResult> Delete([FromBody]AuctionProductStatus model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            public async Task<IActionResult> DeletePermanently([FromBody]AuctionProductStatus model)
            {
                var result = 0;
                if (!(model.Id > 0))
                {
            ...
            [HttpGet]
            [Route("api/Count")]
            public int CountAuctionProductStatus()
            {
            ...
            public async Task<IActionResult> ListServerSide([FromBody] AuctionProductStatusDTParameters parameters)
            {
                try
                {
