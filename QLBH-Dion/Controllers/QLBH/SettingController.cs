    ...
    namespace QLBH_Dion.Controllers
    {
        ...
        public class SettingController : BaseController
        {
            ...
            public SettingController(ISettingService _service, ICacheHelper cacheHelper )
            {
                service = _service;
            }
            [HttpGet]
            ...
            [HttpPost]
            [Route("api/Add")]
            public async Task<IActionResult> Add([FromBody]Setting model)
            {
            ...
            [HttpPost]
            [Route("api/Update")]
            public async Task<IActionResult> Update([FromBody]Setting model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            [HttpPost]
            [Route("api/Delete")]
            public async Task<IActionResult> Delete([FromBody]Setting model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            public async Task<IActionResult> DeletePermanently([FromBody]Setting model)
            {
                var result = 0;
                if (!(model.Id > 0))
                {
            ...
            public async Task<IActionResult> ListServerSide([FromBody] SettingDTParameters parameters)
            {
                try
                {
