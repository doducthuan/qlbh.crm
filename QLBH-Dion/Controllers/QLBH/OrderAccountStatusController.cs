    ...
    namespace QLBH_Dion.Controllers
    {
        [Route("[controller]")]
        [ApiController]
        public class OrderAccountStatusController : BaseController
        {
            IOrderAccountStatusService service;
            public OrderAccountStatusController(IOrderAccountStatusService _service, ICacheHelper cacheHelper )
            {
                service = _service;
            }
            [HttpGet]
            ...
            [HttpPost]
            [Route("api/Add")]
            public async Task<IActionResult> Add([FromBody]OrderAccountStatus model)
            {
            ...
            [HttpPost]
            [Route("api/Update")]
            public async Task<IActionResult> Update([FromBody]OrderAccountStatus model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            [HttpPost]
            [Route("api/Delete")]
            public async Task<IActionResult> Delete([FromBody]OrderAccountStatus model)
            {
                if (ModelState.IsValid)
                {
                    try
            ...
            public async Task<IActionResult> DeletePermanently([FromBody]OrderAccountStatus model)
            {
                var result = 0;
                if (!(model.Id > 0))
                {
            ...
            [HttpGet]
            [Route("api/Count")]
            public int CountOrderAccountStatus()
            {
            ...
            public async Task<IActionResult> ListServerSide([FromBody] OrderAccountStatusDTParameters parameters)
            {
                try
                {
