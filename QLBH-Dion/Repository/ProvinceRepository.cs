    ...
    namespace QLBH_Dion.Repository
        {
            public class ProvinceRepository: IProvinceRepository
                {
                    QLBHContext db;
                    public ProvinceRepository(QLBHContext _db)
                    {
                        db = _db;
                    }
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task <DTResult<Province>> ListServerSide(ProvinceDTParameters parameters)
            {
                //0. Options
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new Province()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<Province>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<Province>()
                {
