    ...
    namespace QLBH_Dion.Repository
        {
            public class OrderAccountPaymentStatusRepository: IOrderAccountPaymentStatusRepository
                {
                    QLBHContext db;
                    public OrderAccountPaymentStatusRepository(QLBHContext _db)
                    {
                        db = _db;
                    }
            ...
            public async Task <List<OrderAccountPaymentStatus>> List()
            {
                    ...
                    {
                        return await(
                            from row in db.OrderAccountPaymentStatuses
                        where(row.Active == 1)
            ...
            public async Task <OrderAccountPaymentStatus> Add(OrderAccountPaymentStatus obj)
            {
                if (db != null) {
                    await db.OrderAccountPaymentStatuses.AddAsync(obj);
            ...
            public async Task Update(OrderAccountPaymentStatus obj)
            {
                if (db != null) {
                    //Update that object
                    db.OrderAccountPaymentStatuses.Attach(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.OrderAccountPaymentStatuses.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.OrderAccountPaymentStatuses.Remove(obj);
            ...
            public async Task <DTResult<OrderAccountPaymentStatus>> ListServerSide(OrderAccountPaymentStatusDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.OrderAccountPaymentStatuses 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new OrderAccountPaymentStatus()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<OrderAccountPaymentStatus>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<OrderAccountPaymentStatus>()
                {
