...
{
    public class OrderAccountStatusRepository : IOrderAccountStatusRepository
    {
        QLBHContext db;
        public OrderAccountStatusRepository(QLBHContext _db)
        {
            db = _db;
        }
        ...
        public async Task<List<OrderAccountStatus>> List()
        {
            ...
            {
                return await (
                    from row in db.OrderAccountStatuses
                    where (row.Active == 1)
        ...
        public async Task<OrderAccountStatus> Add(OrderAccountStatus obj)
        {
            if (db != null)
            {
                await db.OrderAccountStatuses.AddAsync(obj);
        ...
        public async Task Update(OrderAccountStatus obj)
        {
            ...
            {
                //Update that object
                db.OrderAccountStatuses.Attach(obj);
        ...
        public async Task<int> DeletePermanently(int? objId)
        {
            ...
            if (db != null)
            {
                ...
                var obj = await db.OrderAccountStatuses.FirstOrDefaultAsync(x => x.Id == objId);

                if (obj != null)
                {
                    //Delete that obj
                    db.OrderAccountStatuses.Remove(obj);
        ...
        public async Task<DTResult<OrderAccountStatus>> ListServerSide(OrderAccountStatusDTParameters parameters)
        {
            ...
            //1. Join
            var query = from row in db.OrderAccountStatuses
...
EF.Functions.Collate(c.row.CreatedTime.ToCustomString().ToLower(), SQLParams.Latin_General).Contains(EF.Functions.Collate(searchAll, SQLParams.Latin_General))
            ...
            //3.Query second
            var query2 = query.Select(c => new OrderAccountStatus()
            {
                Id = c.row.Id,
                Active = c.row.Active,
                Name = c.row.Name,
            ...
            //4. Sort
            query2 = query2.OrderByDynamic<OrderAccountStatus>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
            recordFiltered = await query2.CountAsync();
            //5. Return data
            return new DTResult<OrderAccountStatus>()
            {
