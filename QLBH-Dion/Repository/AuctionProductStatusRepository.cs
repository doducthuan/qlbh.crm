    ...
    namespace QLBH_Dion.Repository
        {
            public class AuctionProductStatusRepository: IAuctionProductStatusRepository
                {
                    QLBHContext db;
                    public AuctionProductStatusRepository(QLBHContext _db)
                    {
                        db = _db;
                    }
            ...
            public async Task <List<AuctionProductStatus>> List()
            {
                    ...
                    {
                        return await(
                            from row in db.AuctionProductStatuses
                        where(row.Active == 1)
            ...
            public async Task <AuctionProductStatus> Add(AuctionProductStatus obj)
            {
                if (db != null) {
                    await db.AuctionProductStatuses.AddAsync(obj);
            ...
            public async Task Update(AuctionProductStatus obj)
            {
                if (db != null) {
                    //Update that object
                    db.AuctionProductStatuses.Attach(obj);
...
db.Entry(obj).Property(x => x.Description).IsModified = true;
            ...
            public async Task<int> DeletePermanently(int ? objId)
            {
                ...
                if (db != null) {
                    ...
                    var obj = await db.AuctionProductStatuses.FirstOrDefaultAsync(x => x.Id == objId);

                    if (obj != null) {
                        //Delete that obj
                        db.AuctionProductStatuses.Remove(obj);
            ...
            public async Task <DTResult<AuctionProductStatus>> ListServerSide(AuctionProductStatusDTParameters parameters)
            {
                ...
                //1. Join
                var query = from row in db.AuctionProductStatuses 
...
case "createdTime":
                ...
                //3.Query second
                var query2 = query.Select(c => new AuctionProductStatus()
                {
                    Id = c.row.Id,
Active = c.row.Active,
Name = c.row.Name,
...
CreatedTime = c.row.CreatedTime,
                ...
                //4. Sort
                query2 = query2.OrderByDynamic<AuctionProductStatus>(orderCritirea, orderDirectionASC ? LinqExtensions.Order.Asc : LinqExtensions.Order.Desc);
                recordFiltered = await query2.CountAsync();
                //5. Return data
                return new DTResult<AuctionProductStatus>()
                {
